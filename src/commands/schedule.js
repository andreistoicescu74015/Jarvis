...
const confirm = (r) =>
  r.repeatMs
    ? `Scheduled ${r.id}: every ${fmtEvery(r.repeatMs)}, first at ${fmtTime(r.fireAt)}.`
    : `Scheduled ${r.id} for ${fmtTime(r.fireAt)}.`;
...
export default {
  ...
  run: (ctx) => {
    ...
    if (!sub || sub === 'list') {
      ...
      return ['Scheduled:', ...jobs.map((j) => `- ${j.id}: ${describe(j)} -> "${j.text}"`)].join('\n');
    }

    if (sub === 'cancel') {
      const id = (ctx.args[1] ?? '').trim();
      if (!id) return 'Usage: jarvis schedule cancel <id>';
      return ctx.scheduler.cancel(id).ok ? `Cancelled ${id}.` : `No scheduled message "${id}" here.`;
    }

    if (sub === 'in' || sub === 'every') {
      const spec = ctx.args[1];
      const text = ctx.args.slice(2).join(' ').trim();
      if (!spec || !text) return `Usage: jarvis schedule ${sub} <${sub === 'every' ? '1d' : '2h'}> <message>`;
    ...
    if (sub === 'at') {
      ...
      if (!date || !time || !text) return 'Usage: jarvis schedule at <YYYY-MM-DD> <HH:MM> <message>';
      const r = ctx.scheduler.add(`at ${date} ${time}`, text);
      return r.ok ? confirm(r) : whenError(r);
    }

    return 'Usage: jarvis schedule in <2h> <msg> | at <YYYY-MM-DD> <HH:MM> <msg> | every <1d> <msg> | list | cancel <id>';
  },
};
