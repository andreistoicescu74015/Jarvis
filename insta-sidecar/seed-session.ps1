...
# `docker compose --profile instagram up -d` has created the volume. Then restart the sidecar.
...
if (-not $vol) {
    Write-Error "No '*insta-data' Docker volume found. Run 'docker compose --profile instagram up -d' once, then re-run this."
    exit 1
}
...
Write-Host "Done. Now restart the sidecar:  docker compose --profile instagram restart insta-sidecar"
